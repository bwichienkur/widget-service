 class AdAggregator {
    ...
    public async getAds(widgetSettings: any, containerId: string) {
        ...
        if (hasExitPop != null && hasExitPop != '' && hasExitPop == 'true') {
            ...
            if (wasPopShow == null || wasPopShow == 'false') {
                ...
                widgetSettings.TrackId = await this.loadTrackId(widgetSettings);

                if (this.isFormsEngineExist()) {
                    //@ts-ignore
                    $.when(this.readFieldsFromFormsEngine()).then(async function (filterFields) {
                        ...
                        var adList = await fetch('##SERVICEURL##/ExitPop/RenderAd', {
                            method: 'POST',
                            headers: {
    ...
    public async getClientIp() {
        var clientData = await fetch('https://api.bigdatacloud.net/data/ip-geolocation?key=616a14b188da414cb34926d4d1b3616f', {
            method: 'GET',
        });
    ...
    public readFieldsFromFormsEngine() {
        //@ts-ignore
        var searchFilters = $.Deferred();
        var formFilterFields: { [name: string]: number } = {};
    ...
    public async canRender(widgetSettings: string) {

        var trackId = await this.loadTrackId(widgetSettings) as string;

        var canRender = await fetch('##SERVICEURL##/ExitPop/CanRender', {
            method: 'POST',
            headers: {
                'content-type': 'application/json',
            },
    ...
    private async loadTrackId(widgetSettings: string) {
        ...
        if (urlTrackId !== null && urlTrackId !== '')
            ...
            var widgetTrackid = await this.getWidgetTrackId(widgetSettings);
            this.setCookie('_WidgetTrackID', widgetTrackid, 1000);
            return widgetTrackid;
        }
    }

    private async getWidgetTrackId(widgetSettings: string) {
        var adList = await fetch('##SERVICEURL##/ExitPop/GetWidgetTrackId', {
            method: 'POST',
            headers: {
                'content-type': 'application/json',
            },
        ...
        const trackId = await adList.text();
        return trackId
    }
    ...
    private getCookie(coockieName: string): string | null {
